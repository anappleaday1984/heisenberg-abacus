# 海森堡的算盤 · Demo Video v7 · 腳本大綱

> v7 在 v6 之上只做兩處微調,並加一個 PDF 截圖用的深層連結:
> 1. **scene 23 QUOTE 換句**:「不再訪問消費者 — 召喚機率雲」→「資訊是<strong>決定資源分配</strong>的關鍵因素」 — 與 intro-4pages 結尾語統一。
> 2. **開場 banner 拿掉**:start-screen 副標「6 MIN DEMO · 24 場景 · 字體放大 · 桑基交錯版」以及 V6 chip 整段刪,只剩主標 + AIA × CLAUDE CODE SHOWCASE 行 + 播放鈕。
> 3. **深層連結**:加 `?scene=N` / `?t=NN` URL 參數,載入時直接跳到該場景並暫停,start-screen 自動隱藏 — 供 PDF 截圖工具批次撈靜態畫面。
>
> v6 的其他修復(narrator 拆除、字體放大、中英下架、桑基交錯、滷肉飯 4 人設等)v7 全部保留。
>
> 總長 **6 分鐘 / 360 秒 / 24 段**。HTML 端的字幕已隱藏(`.caption { display:none }`),所有旁白文字搬到這個檔,搭配 [demo-video-v7-timeline.json](demo-video-v7-timeline.json) 的時間軸做 TTS 生成。
>
> 對應 HTML: [demo-video-v7.html](demo-video-v7.html)。`SCRIPT_MAP[lang][id].audio` 之後填生成的音檔路徑(`.video` 槽保留但無用)。
>
> 結合版:[demo-final-10min.html](demo-final-10min.html) — intro-4pages(4 分,每分 1 頁)+ demo-video-v7(6 分),總 10 分鐘。

## 推薦輸出目錄

```
public/sound/v7/zh/scene{1..24}.mp3
public/sound/v7/en/scene{1..24}.mp3   # 可選 — UI 鎖 ZH
```
