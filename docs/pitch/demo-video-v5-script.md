# 海森堡的算盤 · Demo Video v5 · 腳本大綱

> v5 在 v4 之上修復 6 個 bug：HUD 6:00 顯示、移除對嘴假動畫、ex-2 卡片改回旅遊保險信用卡、`returning_mom` / `ex_engineer` portrait 重編碼為真 PNG、scene 6 / 9 殘留「10 位」全改 30 位、Sankey 12 條交錯 ribbon 取代原本 6 條直線。**腳本旁白與 timeline 內容沿用 v4。**
>
> 總長 **6 分鐘 / 360 秒 / 24 段**。HTML 端的字幕已隱藏（`.caption { display:none }`），所有旁白文字搬到這個檔，搭配 [demo-video-v5-timeline.json](demo-video-v5-timeline.json) 的時間軸做 TTS / 對嘴生成。
>
> 對應 HTML: [demo-video-v5.html](demo-video-v5.html)。`SCRIPT_MAP[lang][id].audio / .video` 之後填生成的音檔 / mp4 路徑。

## 推薦輸出目錄

```
public/sound/v5/zh/scene{1..24}.mp3
public/sound/v5/en/scene{1..24}.mp3
public/sound/v5/zh/scene{1..24}.mp4   # talking-head（可選）
public/sound/v5/en/scene{1..24}.mp4
```
