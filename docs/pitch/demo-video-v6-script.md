# 海森堡的算盤 · Demo Video v6 · 腳本大綱

> v6 在 v5 之上做整檔放大與多項修復(對齊 z_setting.md 修復清單):
> 1. **整頁字體放大** — `.browser` 加 `zoom: 1.18`,HUD 字級同步加碼。
> 2. **完全移除右下角對嘴 narrator widget** — DOM、CSS、JS 全砍,保留 `SCRIPT_MAP` 純資料供 TTS 串接。
> 3. **移除中英切換按鈕** — `currentLang` 鎖定 `'zh'`,EN 資料保留在 SCRIPT_MAP 不渲染。
> 4. **LIVE 鍵 0:00 點擊** 直接跳到主介面 scene,移除 grok 殘留提示,預設卡片立即顯示。
> 5. **名人肖像模糊** — 加 `.celeb-blur` 加 8px blur + `肖像模糊` 浮水印。
> 6. **二度就業媽媽 / 前科技工程師頭像** 於 scene-persona-show 與 scene-10 雙處可見,尺寸與其他頭像同步放大(64→92px)。
> 7. **「30 位 / 10 位 受訪者」全部改 N 位** — UI、SCRIPT_MAP zh 旁白同步。
> 8. **桑基交錯** — 14 條 ribbon,Layer1↔Layer2 與 Layer2↔Layer3 都有跨層連線,bezier 控制點拉開製造交錯。
> 9. **scene-10 滷肉飯洋芋片** 新增 4 位人設 reaction(外送員 / 上班族 / 二度就業媽媽 / 前科技工程師),每人一條 opinion bubble。
>
> 總長 **6 分鐘 / 360 秒 / 24 段**。HTML 端的字幕已隱藏(`.caption { display:none }`),所有旁白文字搬到這個檔,搭配 [demo-video-v6-timeline.json](demo-video-v6-timeline.json) 的時間軸做 TTS 生成。對嘴影片不再需要 — 右下角已無 narrator widget。
>
> 對應 HTML: [demo-video-v6.html](demo-video-v6.html)。`SCRIPT_MAP[lang][id].audio` 之後填生成的音檔路徑(`.video` 槽保留但已無用)。

## 推薦輸出目錄

```
public/sound/v6/zh/scene{1..24}.mp3
public/sound/v6/en/scene{1..24}.mp3   # 可選 — 目前 UI 鎖 ZH
```
...
- **ZH**：海森堡的算盤 — 把市場調查當作量子觀測，N 位虛擬受訪者並行作答。
- **EN**：Heisenberg's Abacus — market research as quantum observation, N synthetic respondents answering in parallel.
...
- **ZH**：N 位 persona 並行 streaming 答題，concurrency 6、bundled JSON 一通 N 題。
...
- **ZH**：翻頁繼續看 Q3 年費敏感度、Q4 競品比較 — 同樣 N 位受訪者橫向掃描。
...
- **ZH**：行為相變散布圖誕生 — N 顆粒子分布在拒絕 / 觀望 / 購買三區，KPI 同步顯示 38 / 34 / 28%。
...
- **ZH**：N 位受訪者 87 秒給回應：試吃意願 78%、願意付 49 元 52%、期望售價 35 到 55 元、獵奇加分 11pp。同畫面下方四位人設給出第一手反應 — 外送員嫌貴、上班族買單、二度就業媽媽看配料表、前科技工程師覺得適合送禮。
...
3. **填回 HTML**
   - 在 `demo-video-v5.html` 的 `SCRIPT_MAP[lang][n]` 物件填 `audio: '/sound/v5/zh/scene1.mp3'` / `video: '/sound/v5/zh/scene1.mp4'`
